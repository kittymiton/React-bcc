...
// 1:初期状態の設定: 最初に、非同期で取得するデータの初期状態として空配列やnullなどをuseStateフックで設定します。これにより、データがまだ取得されていない状態を表現します。
// 2:非同期データの取得: ページ読み込み時に、useEffectフックのコールバック関数内で非同期データの取得を行います。この時、通常はAPIからデータをフェッチする関数を呼び出します。
// 3:データの更新: データの取得が成功すると、useEffectフックのコールバック関数内でuseStateフックの状態更新関数を使用して、取得したデータを状態に設定します。これにより、コンポーネントが再レンダリングされ、画面上に新しいデータが表示されます。
// 4:画面の更新: 状態が更新されると、Reactはコンポーネントを再レンダリングし、画面上に新しいデータが表示されます。このプロセスにより、ユーザーに対して最新の情報が提供されます。

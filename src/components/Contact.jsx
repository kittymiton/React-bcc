...
export const Contact = () => {
  ...
  const validate = () => {
    ...
    // 変数を使用してエラーメッセージを一時的に保存。この変数は関数スコープまたはブロックスコープ内で定義されているため、その作用域内でのみアクセス可能です。この方法は、コンポーネントのレンダリングに直接影響を与えないため、パフォーマンス上の優位性があります。しかしコンポーネント間でエラーメッセージを共有する必要がある場合は、グローバルステート管理やコンテキストAPIなどの手段を必要とするかもしれません。
    let nameLowError = "";
    let emailLowError = "";
    let messageLowError = "";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/contacts`, {
        ...
        body: JSON.stringify({ name, email, message }),// // stateにユーザーからの入力や前処理済みのデータを保持されている値を参照。するために使用。JSON.stringifyは最初の引数にオブジェクトを取り、そのオブジェクトのプロパティを含むJSON文字列を生成。複数の変数を直接渡すことはできません。オブジェクトを形成してからJSON.stringifyを呼び出す必要があります。
      });
  ...
  return (
    <main>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          {/* <button type="submit">送信</button> {/* formの送信をトリガー。type="submit"属性を使用するとボタンがフォームの一部として機能し、ユーザーがフォームを送信。value属性は、ボタンがクリックされたときに送信されるデータの値を指定するために使用。value="submit"属性は送信される値。この場合submitが送信されたデータの値として扱われます。サーバー側ではsubmitの値（"Submit"）も一緒に受け取ることができます。submitの値は、フォームが送信された動機や、送信ボタンがクリックされたときの追加情報として使用されることがあります。このように、value="submit"のvalue属性は、フォームが送信されるときにサーバーに送られるデータの一部となります。これにより、サーバー側でフォームの送信元や動機など、さらに詳細な情報を把握することが可能になります */}
          <Button
            type="submit"
            tag="送信"
          />
